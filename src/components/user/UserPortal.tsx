...
function UserAuth() {
  ...
  const submit = async (event: FormEvent) => {
    ...
    setError("");
    try {
      if (mode === "register") await register(name.trim(), email.trim(), password, { phone: phone.trim(), city: city.trim(), postalCode: postalCode.trim() });
...
function SettingsPage({ notify }: { notify: (message: string) => void }) {
  return <div className="user-standard-page narrow"><div className="user-page-heading"><div><span className="user-kicker">Your account</span><h2>Settings</h2><p>Manage your personal information and contact details.</p></div></div><ProfileSettings notify={notify} /><section className="user-surface"><span className="user-kicker">Preferences</span><h3>Conversation experience</h3><label className="user-setting-toggle"><span><strong>Save conversation history</strong><small>Keep chats available across your devices.</small></span><input type="checkbox" defaultChecked /><i /></label><label className="user-setting-toggle"><span><strong>Weekly reflection email</strong><small>Receive one thoughtful prompt each week.</small></span><input type="checkbox" defaultChecked /><i /></label><label className="user-setting-toggle"><span><strong>Product updates</strong><small>Hear about new masters and features.</small></span><input type="checkbox" /><i /></label></section></div>;
}
...
export function UserPortal({ path }: { path: string[] }) {
  ...
  const titles: Record<string, string> = { home: "My Space", teachers: "Masters", chat: "Conversation", history: "History", usage: "Usage", plan: "My Plan", billing: "Billing", settings: "Settings" };
